...
module.exports = function(grunt) {
    ...
    dirs.dest_styles_c      = dirs.dest_styles  + '/c';
    dirs.dest_styles_p      = dirs.dest_styles  + '/p';
    ...
    grunt.initConfig({
        ...
        concat: {
            ...
            require_js: {
                src: [
                    dirs.node_modules + '/requirejs/require.js'
                ],
                dest: dirs.dest_scripts_pkg + '/require.js'
            }
        },
        ...
        uglify: {
            ...
            require_js: {
                files: [
                    {
                        src: dirs.dest_scripts_pkg + '/require.js',
                        dest: dirs.dest_scripts_min + '/require.js'
                    }
                ]
            }
        },
        ...
        copy: {
            ...
            dest_styles_to_dest_styles_c: {
                options: {
                    process: function(content) {
                        content = content.replace(/url\s*\(['"]?\.\.\/images\/(.*?)['"]?\)/gi, function(g, s1) {
                            return 'url("../../i/' + s1 + '")';
                        });
                        return content;
                    }
                ...
                files: [
                    {
                        expand: true,
                        cwd: dirs.dest_styles + '/',
                        src: ['**'],
                        dest: dirs.dest_styles_c + '/'
                    }
                ]
            },
            dest_pages_to_dest_styles_p: {
                options: {
                    process: function(content) {
                        content = content.replace(/src\s*=\s*['"]\.\.\/images\/(.*?)['"]/gi, function(g, s1) {
                            return 'src="../../i/' + s1 + '"';
                        });
                        content = content.replace(/href\s*=\s*['"]\.\.\/styles\/(.*?)['"]/gi, function(g, s1) {
                            return 'href="../c/' + s1 + '"';
                        });
                        content = content.replace(/src\s*=\s*['"](.*?)\.js['"]/gi, function(g, s1) {
                            return 'src="http://jsdev7.m.sohu.com/h5/js/tags/' + pkg.name.split('-')[1] + '/' + pkg.name.split('-')[0] + '/test/' + s1.substring(s1.lastIndexOf('\/') + 1) + '.js"';
                        });
                        content = content.replace(/data-main\s*=\s*['"]\.\.\/scripts\/(.*?)['"]/gi, function(g, s1) {
                            return 'data-main="http://jsdev7.m.sohu.com/h5/js/tags/' + pkg.name.split('-')[1] + '/' + pkg.name.split('-')[0] + '/test/' + s1 + '"';
                        });
                        content = content.replace(/baseImagePath\s*=\s*['"]\.\.\/images\/['"]/gi, function(g) {
                            return 'baseImagePath="../../i/"';
                        });
                        return content;
                    }
                },
    ...
    grunt.registerTask('build-css', ['build-css-base', 'less:build', 'copy:app_images_to_dest_images', 'copy:dest_pages_pkg_to_dest_pages', 'copy:dest_styles_to_dest_styles_c', 'copy:dest_pages_to_dest_styles_p', 'clean:dest_pages_pkg']);
    ...
    grunt.registerTask('release', ['clean:dest', 'clean:release',
        ...
        'build-css-base', 'less:release', 'imagemin', 'base64image', 'cssmin:release', 'copy:dest_styles_to_dest_styles_c', 'copy:dest_pages_to_dest_styles_p', 'clean:release']);
